...
Данный проект реализует лабораторную работу №6, посвященную изучению **пользовательских аннотаций (Custom Annotations)**, **Reflection API** и обработке метаданных в языке программирования **Java**.
...
Или запустите класс `shurmanov.main.Main` из IDE (IntelliJ IDEA, Eclipse).
...
Для запуска тестов используется JUnit 5 (Jupiter):

```bash
javac -cp junit-jupiter-api-5.9.0.jar:junit-jupiter-params-5.9.0.jar -d out $(find src -name "*.java")
```
...
- **Java 8+** (использование современных возможностей)
- **Reflection API** — для работы с аннотациями в рантайме
- **Annotations (JDK)** — @Target, @Retention, @interface
- **Enum** — для создания типобезопасных перечислений (Mode)
- **JUnit 5 (Jupiter)** — для написания и запуска тестов
- **Collections** — Arrays для работы с массивами и Stream API

---

# Что можно улучшить дальше

- Добавить JUnit тесты для всех остальных аннотаций (не только @Default)
- Реализовать более сложные аннотации с дополнительными параметрами (String, int, boolean)
- Создать генерик-обработчик для автоматизации поиска аннотаций
- Добавить обработку вложенных аннотаций (@Repeatable)
- Использовать функциональный подход (Stream API) при обработке методов и полей
- Реализовать паттерн Decorator с помощью аннотаций для изменения поведения методов

---

# Советы по отладке

- Всегда проверяйте `@Retention(RetentionPolicy.RUNTIME)` — без этого аннотация не будет видна в рантайме
- Используйте `getDeclaredMethods()` / `getDeclaredFields()` для доступа к приватным членам класса
- Помните про `setAccessible(true)` при работе с приватными полями через Reflection
- Логируйте исключения из `InvocationTargetException.getCause()` — там реальная причина ошибки
- Используйте IDE для проверки синтаксиса аннотаций (часто там видны ошибки)
